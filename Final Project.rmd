...
rng_count_by_year %>%
  ...
  ggplot(aes(x=as.character(operational_year), y=n, fill=rng_type)) + geom_bar(stat='identity') + labs(title="Project Count by Year", fill="RNG Type", x="Operatonal Year", y="Project Count") + theme(legend.position = "bottom", plot.title = element_text(hjust = 0.5))
```

### RNG Project Count by State
...
rng_count_by_state %>%
  ggplot(aes(x=reorder(state, n), y=n, fill=rng_type, )) + geom_bar(stat='identity') + labs(title="Project Count by State", fill="RNG Type", x="State", y="Project Count") + theme(legend.position = "bottom", plot.title = element_text(hjust = 0.5)) + coord_flip()
```
