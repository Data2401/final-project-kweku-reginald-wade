...
arboc_issuance_nice_names = arboc_issuance_nice_names %>%
  mutate(arb_offset_credits_issued = strtoi(arb_offset_credits_issued))
...
```
### Credits Issued by Project

```{r}
options(scipen=10000)

arboc_issuance_clean %>%
  select(opr_project_id, project_type_desc, arb_offset_credits_issued) %>% 
  group_by(opr_project_id, project_type_desc) %>%
  head(10) %>% 
  summarise(total_credits = sum(arb_offset_credits_issued, na.rm = T)) %>%
  ggplot(aes(x=reorder(opr_project_id, -total_credits), y=total_credits, color=project_type_desc)) + geom_point()
```
